...
cmake -S "$ROOT_DIR/test" -B "$ROOT_DIR/test/build-off" -DIP_ENABLE=OFF -DCMAKE_BUILD_TYPE=Release
cmake --build "$ROOT_DIR/test/build-off" --target workload

cmake -S "$ROOT_DIR/test" -B "$ROOT_DIR/test/build-on" -DIP_ENABLE=ON -DCMAKE_BUILD_TYPE=Release
